...
def wisdom_coverage_change(args, model, cluster_method_name, device, wisdom_k_neurons, train_loader, clean_data, mixed_data):
    cache_path = (
        "./cluster_pkl/"
    ...
    clean_loader = DataLoader(clean_data, batch_size=args.batch_size, shuffle=False)
    clean_acts = wisdom_idc.get_selected_activations(clean_loader, wisdom_k_neurons)
    clean_coverage, _, _ = wisdom_idc.compute_coverage(clean_acts, cluster_groups)
    

    wisdom_idc.total_combination = 1
    mixed_loader = DataLoader(mixed_data, batch_size=args.batch_size, shuffle=False)
    mixed_coverage, _, _ = wisdom_idc.compute_coverage(mixed_loader, wisdom_k_neurons, cluster_groups)
                ...
                for attack_ in attack_names:
                    ...
                    for method_name, template in base_templates.items():
                        if method_name == 'DeepImportance':
                            ...
                            coverage_change = wisdom_coverage_change(args, model, "KMeans", device, wisdom_k_neurons, train_loader, clean_dataset, mixed_dataset)
                        else:
