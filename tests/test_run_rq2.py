"""Tests for run_rq2.py – diversity evaluation."""
import os
import tempfile
import pytest
...
@skip_missing
def test_rq2_generates_csv():
    import sys, pathlib, csv
    sys.path.insert(0, str(pathlib.Path(__file__).resolve().parent.parent))
    from run_cases.run_rq2 import run_rq2

    device = "cuda:0" if __import__("torch").cuda.is_available() else "cpu"
    with tempfile.TemporaryDirectory() as tmpdir:
        out = os.path.join(tmpdir, "rq2.csv")
        result = run_rq2(
            weights=WEIGHTS, img_dir=COCO_VAL, csv_file=SCORES_CSV,
            out_csv=out, device=device, num_images=4, batch_size=2,
            imgsz=320, n_iterations=1,
        )
        assert os.path.isfile(result)
        with open(result) as f:
            rows = list(csv.DictReader(f))
        assert len(rows) > 0
        assert "Perturbation" in rows[0]
