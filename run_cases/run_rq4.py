...
def get_yolo_predictions(model: nn.Module, images: torch.Tensor, device: str,
                         batch_size: int = 16, num_classes: int | None = None) -> List[int]:
    ...
    with torch.no_grad():
        for i in range(0, len(images), batch_size):
            chunk = images[i:i + batch_size]
            preds = normalize_detection_output(model(chunk.to(device)), num_classes=num_classes)
...
def run_rq4(
    ...
    try:
        ...
        axes[0].scatter(df["Pielou Evenness"], df["WISDOM Coverage"], color="#5a4491", alpha=0.8, label="WISDOM")
        axes[0].set_xlabel("Pielou Evenness")
        axes[0].set_ylabel("WISDOM Coverage")
        axes[0].set_title("RQ4: Evenness vs WISDOM Coverage")
        axes[0].grid(True, alpha=0.3)

        axes[1].scatter(df["Pielou Evenness"], df["Neuron Coverage"], color="#d0beed", alpha=0.8, label="Neuron Coverage")
        axes[1].set_xlabel("Pielou Evenness")
        axes[1].set_ylabel("Neuron Coverage")
        axes[1].set_title("RQ4: Evenness vs Neuron Coverage")
