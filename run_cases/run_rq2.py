...
def run_rq2(
    ...
    try:
        ...
        grouped = df.groupby(["Iteration", "Perturbation"])["Coverage"].mean().unstack(fill_value=0.0)
        grouped.plot(kind="bar", ax=ax, color=["#5a4491", "#d0beed"])
        ax.set_ylabel("Coverage")
        ax.set_title("RQ2 Coverage by Iteration")
        ax.grid(True, axis="y", alpha=0.3)
        fig.tight_layout()
        iter_plot_path = f"{plot_base}_plot.pdf"
        fig.savefig(iter_plot_path, format="pdf", dpi=1200, bbox_inches="tight")
        plt.close(fig)

        fig, ax = plt.subplots(figsize=(6.5, 4.8))
        box_data = [
            df[df["Perturbation"] == "Important (U_I)"]["Coverage"].astype(float).tolist(),
            df[df["Perturbation"] == "Random (U_R)"]["Coverage"].astype(float).tolist(),
        ]
        bp = ax.boxplot(box_data, labels=["Important (U_I)", "Random (U_R)"], patch_artist=True, showfliers=False)
        for patch, color in zip(bp["boxes"], ["#5a4491", "#d0beed"]):
            patch.set(facecolor=color, edgecolor="#2b1f4a", linewidth=0.8)
        for median in bp["medians"]:
            median.set(color="#1f1f1f", linewidth=1.0)
        ax.set_ylabel("Coverage")
        ax.set_title("RQ2 Coverage Distribution")
        ax.grid(True, axis="y", alpha=0.3)
        fig.tight_layout()
        box_plot_path = f"{plot_base}_boxplot.pdf"
        fig.savefig(box_plot_path, format="pdf", dpi=1200, bbox_inches="tight")
        plt.close(fig)
        print(f"Plot saved: {iter_plot_path}")
        print(f"Plot saved: {box_plot_path}")
...
if __name__ == "__main__":
    ...
    p.add_argument("--n-iterations", type=int, default=NUM_ITERATIONS)
    args = p.parse_args()
    run_rq2(**vars(args))
