...
def test_cifar10_all(attributions, args, model, trainloader, testloader, classes, net_layer, layer_name):
    ...
    # To store the accuracy difference for each class
    accuracy_diff = {key: {} for key in classes}
    ...
    for single_class in classes:
        ...
        for attr in attributions:
            ...
            accuracy_diff[single_class][attr] = b_accuracy - a_accuracy
    ...
    # Find the attribution method with the highest accuracy difference for each class
    best_attributions = {key: max(value, key=value.get) for key, value in accuracy_diff.items()}
    with open('best_attributions_{}_{}.json'.format(module_name[args.layer_index], args.top_m_neurons), 'w') as log_file:
        json.dump(best_attributions, log_file, indent=4)
